...
Create a lightweight DQL session pinned to a target cluster id.

```typescript
session(clusterId: string): MilvusClientSession
```

The session reuses the parent client's connection and injects `cluster_id` into supported requests. Closing the session prevents future session calls, but it does not close the parent `MilvusClient` or the gRPC channel pool.

Supported session methods:

| Method                   | Description                                           |
| ------------------------ | ----------------------------------------------------- |
| `search(params)`         | Search with the session cluster id.                   |
| `hybridSearch(params)`   | Hybrid search with the session cluster id.            |
| `searchIterator(params)` | Create a search iterator with the session cluster id. |
| `query(params)`          | Query with the session cluster id.                    |
| `queryIterator(params)`  | Create a query iterator with the session cluster id.  |
| `get(params)`            | Get rows by primary key with the session cluster id.  |
| `close()`                | Close only this session wrapper.                      |

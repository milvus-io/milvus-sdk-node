...
Used for BM25, text embedding, rerank, MinHash, and other function schemas.
...
`addFunctionField()` currently provides validated convenience wrappers for
`FunctionType.BM25` and `FunctionType.MINHASH` schema evolution.
...
### SearchAggregation <span className="version-tag">Milvus 3.0</span>

```typescript
type AggregationMetricOp = 'avg' | 'sum' | 'count' | 'min' | 'max';
type AggregationDirection = 'asc' | 'desc';

interface SearchAggregation {
  fields: string[];
  size: number;
  metrics?: Record<string, { op: AggregationMetricOp; field_name: string }>;
  order?: { key: string; direction: AggregationDirection }[];
  top_hits?: {
    size: number;
    sort?: {
      field_name: string;
...
interface AggregationBucket {
  key: { field_id: string | number; field_name: string; value?: unknown }[];
  count: string | number;
...
See [User & Role Operations](/milvus-sdk-node/api-reference/users-roles) for RBAC methods and examples.

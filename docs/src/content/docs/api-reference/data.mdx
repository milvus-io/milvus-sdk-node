...
- `group_by_fields?`: Scalar fields used to group aggregation results.
- `order_by_fields?`: Sort fields, such as `'price:asc'` or `[{ field: 'price', order: 'asc' }]`.
- `order_by?`: Alias for `order_by_fields`.
- `params?`: Additional query parameters forwarded to Milvus.
- `consistency_level?`: Consistency level.
...
Specify aggregation expressions in `output_fields`. Use `group_by_fields` to
return one row per scalar group. Supported aggregation functions are `count`,
`min`, `max`, `sum`, and `avg`.
...
Without `group_by_fields`, aggregation expressions return collection-level
values. Response keys use the expressions supplied in `output_fields`.
...
- `search_aggregation?`: Bucket aggregation configuration.
- `db_name?`: Database name.
- `timeout?`: Request timeout in milliseconds.
...
`search_aggregation` groups search candidates into buckets and can calculate
metrics, return sorted top hits, and recursively create nested buckets.
...
interface SearchAggregation {
  ...
  metrics?: Record<
    string,
    { op: 'avg' | 'sum' | 'count' | 'min' | 'max'; field_name: string }
  >;
...
A single query vector returns `AggregationBucket[]`; multiple query vectors
return `AggregationBucket[][]`. Aggregation searches return their grouped data
through `agg_buckets` instead of regular hit rows. Search aggregation cannot be combined with
hybrid search, highlighters, `group_by_field`, `group_by_fields`, or a positive
offset.
...
interface SearchResults<T> {
  ...
  agg_buckets?: AggregationBucket[] | AggregationBucket[][];
}
...
- [Insert & Update guide](/milvus-sdk-node/operations/data-operations-insert)
- [Query & Search guide](/milvus-sdk-node/operations/data-operations-query)
- [Index Operations](/milvus-sdk-node/api-reference/indexes)

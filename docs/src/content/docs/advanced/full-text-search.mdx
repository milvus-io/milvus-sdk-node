...
For analyzers that load a custom dictionary or synonym file, register the
server-visible object before referencing it from analyzer parameters:
...
The file must already be available through the storage configured for Milvus.
`addFileResource()` registers metadata; it does not upload a local Node.js file.
Remove the registration when it is no longer used:
...
You can add the BM25 output field, function, and bound index to an existing
collection in one schema alteration:
...
- Learn about [Hybrid Search](/milvus-sdk-node/operations/hybrid-search) for combining full-text with vector search
- Explore [Data Types & Schemas](/milvus-sdk-node/core-concepts/data-types-schemas) for SparseFloatVector details

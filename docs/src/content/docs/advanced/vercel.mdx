...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { collectionName, vector, limit = 10, outputFields = [] } = req.body;

    if (!collectionName || !vector) {
      return res.status(400).json({
        error: 'Missing required fields: collectionName and vector',
      });
    }
...
export async function POST(request) {
  try {
    ...
    const { collectionName, vector, limit = 10, outputFields = [] } = body;
...
export default async function handler(req, res) {
  ...
  try {
    const milvusClient = getClient();
    const { collectionName, vectors, limit = 10, outputFields = [] } = req.body;
...
export default async function handler(req, res) {
  ...
  try {
    const milvusClient = getClient();
    const { collectionName, filter, outputFields = [] } = req.body;

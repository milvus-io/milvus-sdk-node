...
HTTP client requests use camelCase field names such as `collectionName`, `dbName`, and `outputFields`. The gRPC `MilvusClient` uses snake_case names such as `collection_name` and `output_fields`.
...
```javascript
const result = await client.createImportJobs({
  collectionName: 'my_collection',
  files: [['imports/data.json']],
});

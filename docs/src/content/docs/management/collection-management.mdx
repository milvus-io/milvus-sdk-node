...
You can also pass `field_id` instead of `field_name`. Get the field ID from
`describeCollection({ cache: false })` and provide exactly one identifier.
...
### Add a Function Output Field and Index

<span className="version-tag">Milvus 3.0</span>

Use `addFunctionField()` to atomically add a function output field, the function
schema, and its bound index. The wrapper supports BM25 and MinHash functions.
...
MinHash functions generate binary signatures for set or shingle similarity:
...
The lower-level `alterCollectionSchema()` API exposes the same schema mutation
when you need to build the complete request yourself. The older
`addCollectionFunction()` method remains available, but is deprecated for this
schema-evolution workflow.

### Drop a Function and Its Output Field
...
`dropFunctionField()` also removes the function output fields and bound indexes.
The older `dropCollectionFunction()` method does not provide this cascade and is
deprecated for schema-evolution workflows.

...
Milvus 3.0 can generate MinHash binary signatures from a text field and create
the bound index atomically:
...
Use `MHJACCARD` for MinHash function output. Plain binary-vector workflows can
continue to use `JACCARD` when appropriate.

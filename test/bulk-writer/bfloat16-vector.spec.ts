...
describe('BFloat16Vector Validation', () => {
  const dim = 4;
  const testArray = [1.0, 2.0, 3.0, 4.0];

  describe('Array Input', () => {
    it('should validate and convert array to Uint8Array', () => {
      const result = validateBFloat16Vector(testArray, dim);

      expect(result.value).toBeInstanceOf(Uint8Array);
      expect(result.size).toBe(dim * 2); // 2 bytes per dimension for bf16
      expect(result.value.length).toBe(8);
    });

    it('should throw error for wrong dimension', () => {
      expect(() => {
        validateBFloat16Vector(testArray, 2); // Wrong dimension
      }).toThrow('Invalid float vector: expected array with dim=2');
    });

    it('should handle empty array', () => {
      const result = validateBFloat16Vector([], 0);
      expect(result.value).toBeInstanceOf(Uint8Array);
  ...
  describe('Base64 String Input', () => {
    it('should validate and convert base64 string to Uint8Array', () => {
      ...
      const arrayResult = validateBFloat16Vector(testArray, dim);
      const base64String = Buffer.from(arrayResult.value).toString('base64');

      const result = validateBFloat16Vector(base64String, dim);
    ...
    it('should throw error for invalid base64 string', () => {
      expect(() => {
        validateBFloat16Vector('invalid-base64!@#', dim);
      }).toThrow('Invalid base64 string for BFloat16Vector');
    });

    it('should throw error for base64 with wrong length', () => {
      expect(() => {
        validateBFloat16Vector('AA==', dim); // 1 byte, should be 8 bytes
      }).toThrow(
        'Invalid BFloat16Vector base64: expected length 8 bytes, got 1'
      );
    });
  });
  ...
  describe('Uint8Array Input', () => {
    it('should validate existing Uint8Array', () => {
      ...
      const result = validateBFloat16Vector(uint8Array, dim);
    ...
    it('should throw error for Uint8Array with wrong length', () => {
      ...
      expect(() => {
        validateBFloat16Vector(wrongArray, dim);
  ...
  describe('Round-trip Conversion', () => {
    it('should maintain data integrity through array -> bytes -> base64 -> bytes conversion', () => {
      ...
      // Base64 -> bytes
      const base64Result = validateBFloat16Vector(base64String, dim);
  ...
  describe('Error Handling', () => {
    it('should throw error for null input', () => {
      expect(() => {
        validateBFloat16Vector(null as any, dim);
      }).toThrow('Invalid float vector: expected array with dim=4');
    });

    it('should throw error for undefined input', () => {
      expect(() => {
        validateBFloat16Vector(undefined as any, dim);
      }).toThrow('Invalid float vector: expected array with dim=4');
    });

    it('should throw error for non-array object', () => {
      expect(() => {
        validateBFloat16Vector({ key: 'value' } as any, dim);
      }).toThrow('Invalid float vector: expected array with dim=4');
    });
  });
});
